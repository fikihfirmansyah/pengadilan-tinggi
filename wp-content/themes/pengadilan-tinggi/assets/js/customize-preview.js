/* global pengadilanTinggiBgColors, pengadilanTinggiPreviewEls, jQuery, _, wp */
/**
...
(function ($, api, _) {
		...
		value.bind(function () {
			...
			setTimeout(function () {
				Object.keys(pengadilanTinggiBgColors).forEach(function (context) {
					pengadilanTinggiGenerateColorA11yPreviewStyles(context);
				});
			}, 50);
		});
	});
	 ...
	 * @return {void}
	 */
	function pengadilanTinggiGenerateColorA11yPreviewStyles(context) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize('accent_accessible_colors').get(),
			stylesheedID = 'pengadilantinggi-customizer-styles-' + context,
			stylesheet = $('#' + stylesheedID),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if (!stylesheet.length) {
			$('#pengadilantinggi-style-inline-css').after('<style id="' + stylesheedID + '"></style>');
			stylesheet = $('#' + stylesheedID);
		}
		if (!_.isUndefined(a11yColors[context])) {
			// Check if we have elements defined.
			if (pengadilanTinggiPreviewEls[context]) {
				_.each(pengadilanTinggiPreviewEls[context], function (items, setting) {
	...
	$(function () {
		pengadilanTinggiGenerateColorA11yPreviewStyles('content');
		pengadilanTinggiGenerateColorA11yPreviewStyles('header-footer');
	});
