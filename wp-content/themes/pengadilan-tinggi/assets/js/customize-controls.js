/* global pengadilanTinggiBgColors, pengadilanTinggiColor, jQuery, wp, _ */
/**
...
(function () {
	...
	wp.customize.bind('ready', function () {
		...
		wp.customize('accent_hue', function (value) {
			value.bind(function (to) {
				...
				Object.keys(pengadilanTinggiBgColors).forEach(function (context) {
					var backgroundColorValue;
					if (pengadilanTinggiBgColors[context].color) {
						backgroundColorValue = pengadilanTinggiBgColors[context].color;
					} else {
						backgroundColorValue = wp.customize(pengadilanTinggiBgColors[context].setting).get();
					}
					pengadilanTinggiSetAccessibleColorsValue(context, backgroundColorValue, to);
				});
			});
		});
		...
		Object.keys(pengadilanTinggiBgColors).forEach(function (context) {
			wp.customize(pengadilanTinggiBgColors[context].setting, function (value) {
				value.bind(function (to) {
					// Update the value for our accessible colors for this area.
					pengadilanTinggiSetAccessibleColorsValue(context, to, wp.customize('accent_hue').get(), to);
				});
			});
		});

		// Show or hide retina_logo setting on the first load.
		pengadilanTinggiSetRetineLogoVisibility(!!wp.customize('custom_logo')());

		// Add a listener for custom_logo changes.
		wp.customize('custom_logo', function (value) {
			value.bind(function (to) {
				// Show or hide retina_logo setting on changing custom_logo.
				pengadilanTinggiSetRetineLogoVisibility(!!to);
			});
		});
	});
	 ...
	 * @return {void}
	 */
	function pengadilanTinggiSetAccessibleColorsValue(context, backgroundColor, accentHue) {
		var value, colors;
		...
		colors = pengadilanTinggiColor(backgroundColor, accentHue);
	 ...
	 *
	 * @since Pengadilan Tinggi 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function pengadilanTinggiSetRetineLogoVisibility(visible) {
